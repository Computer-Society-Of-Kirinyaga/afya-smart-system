...
function VitalsPage() {
  ...
  const currentValue = streamData
    ? ((streamData[
        VITAL_TO_API_KEY[activeTab] as keyof typeof streamData
      ] as number) ?? 0)
    : activeTab === 'heartRate'
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(reversedHistory.length / ROWS_PER_PAGE),
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="bg-linear-to-br from-teal-500 to-teal-700 rounded-xl p-8 text-white shadow-lg"
      >
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
        ...
        <div className="h-44 bg-linear-to-b from-teal-50/50 to-white rounded-lg flex items-end gap-px p-3">
