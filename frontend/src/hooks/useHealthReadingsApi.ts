...
export function useLatestHealthReading(userId: string) {
  return useQuery({
    ...
    queryFn: () =>
      api.get<HealthReading>(`/health-readings/latest-health/${userId}`),
    enabled: !!userId,
...
export function useHealthReadings(userId: string) {
  return useQuery({
    ...
    queryFn: () =>
      api.get<HealthReading[]>(`/health-readings/health-reading/${userId}`),
    enabled: !!userId,
  ...
  return useQuery({
    ...
    queryFn: () =>
      api.get<Record<string, unknown>>(
        `/health-readings/latest-health/${userId}`,
      ),
    enabled: !!userId,
