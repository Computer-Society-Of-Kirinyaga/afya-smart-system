...
**Tabs usage:**
```tsx
<Tabs defaultValue="heartRate" onValueChange={(v) => setSelectedVital(v as VitalType)}>
  ...
  {benchmarks.map(b => (
    <div key={b.vital}>
      <div className="mb-1 flex items-center justify-between text-sm">
        ...
        <span className="font-medium text-slate-900">{b.percentile}th percentile</span>
      </div>
      <Progress
        value={b.percentile}
...
<div className="flex items-center justify-between py-3">
  <div>
    <p className="text-sm font-medium text-slate-900">SMS for Critical Alerts</p>
    <p className="text-xs text-slate-500">Send SMS when a critical threshold is breached</p>
  </div>
  <Switch
